...
Router.get('/experiences', (req, res) => {
	experiences.find().select('slug img_thumb caption title type best_time_to_visit best_month_to_visit latitude longitude filter zone').exec(function (err, experiences) {
		if (err) {
			console.log('error finding trips for home')
		} else {
...
Router.get("/place/:slug", (req, res) => {
	places.findOne({
		...
		if (err || data == null) {
			...
			experiences.find({ "id": { $in: data.experiences } }).select('slug title name caption tags img img_thumb').exec(function (err, _experiences) {
				if (err) {
					console.error(err);
				} else {
					mustCarry.find({ "id": { $in: data.must_carry } }).select('slug title source information').exec(function (err, _must_carry) {
